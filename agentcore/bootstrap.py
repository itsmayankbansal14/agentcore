...
def is_supported_python() -> bool:
    """Return True only for Python 3.11.x or 3.12.x."""
    return MIN_PY <= sys.version_info[:2] < MAX_PY_EXCLUSIVE


def _find_supported_python() -> str | None:
    """Try to find a supported Python (3.11 or 3.12) on Windows using the py launcher."""
    if os.name != "nt":
        return None
